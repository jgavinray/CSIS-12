...
;---------------------------------------------------------
ReadFloat_old PROC USES eax ebx ecx
   LOCAL expsign:SDWORD, sign:byte
;
; This version is deprecated (11/20/09). The updated version 
; is shorter after having had redundant lines removed.
;----------------------------------------------------------
.data
expint SDWORD  0
itmp       SDWORD  ?
...
R2:
    ...
    .IF (al >= '0' && al <= '9') || (al == '.')	
      ...
      .IF (al == '.')
        ...
        fadd       ; add the front end to the back end
      .ENDIF
; *** Comment by Mike Byrne: Lines 142-159 can be eliminated by 
; *** changing the IF statement in Line 108 to:  .IF (al >= '0' && al <= '9') || (al == '.')			
Comment @
    .ELSEIF (al == '.')
      call GetChar
		  ; something like ".ddd"
